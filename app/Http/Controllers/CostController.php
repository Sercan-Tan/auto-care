...
class CostController extends Controller
{
    ...
    private function getMonthlyCosts($startDate, $endDate)
    {
        ...
        $yearFn = $driver === 'sqlite' ? "strftime('%Y', service_date)" : "YEAR(service_date)";
        $monthFn = $driver === 'sqlite' ? "strftime('%m', service_date)" : "MONTH(service_date)";

        $laborCostsByMonth = Service::whereBetween('service_date', [$startDate, $endDate])
            ->selectRaw("SUM(labor_cost) as total, $yearFn as year, $monthFn as month")
            ->groupBy('year', 'month')
            ->orderBy('year')
            ->orderBy('month')
            ->get();
        ...
        $partCostsByMonth = DB::table('service_items')
            ...
            ->selectRaw("SUM(service_items.part_cost) as total, $yearFn as year, $monthFn as month")
            ->groupBy('year', 'month')
            ->orderBy('year')
            ->orderBy('month')
            ->get();
